...
def build_poison_generator(config):
    """Construct the generator recorded by a PoisonGANConfig/checkpoint."""
    common = {
    ...
    if config.gan_variant == "legacy":
        return PoisonFeatureGenerator(
            audio_out_max=config.audio_out_max,
            **common,
        )
    if config.gan_variant == "temporal_adaptive":
    ...
    raise ValueError(f"Unknown GAN variant: {config.gan_variant}")
